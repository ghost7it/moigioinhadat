...
namespace Web.Areas.Management.Controllers
{
    ...
    {
        [Route("bai-viet-dang-bien-tap", Name = "ArticleIndex")]
        [ValidationPermission(Action = ActionEnum.Read, Module = ModuleEnum.Article)]
        public ActionResult Index()
        {
            return View();
        }
        [Route("bien-tap-bai-viet", Name = "ArticleCreate")]
        [ValidationPermission(Action = ActionEnum.Create, Module = ModuleEnum.Article)]
        ...
        [ValidateInput(false)]
        [Route("bien-tap-bai-viet")]
        [ValidationPermission(Action = ActionEnum.Create, Module = ModuleEnum.Article)]
        public async Task<ActionResult> Create(ArticleCreatingViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                int result = await _repository.GetRepository<Article>().CreateAsync(article, AccountId);
                if (result > 0)
                {
                    if (model.CategoryId != null && model.CategoryId.Any())
                    {
                        ...
                        try
                        {
                            int result2 = await _repository.GetRepository<ArticleCategory>().CreateAsync(articleCategory, AccountId);
                        }
                        catch { }
                    }
                    TempData["Success"] = "Nhập bài viết mới thành công!";
                ...
                else
                {
                    //ViewBag.Categories = await _repository.GetRepository<Category>().GetAllAsync(o => o.CategoryId == null);
                    ViewBag.CategoryTypes = await _repository.GetRepository<CategoryType>().GetAllAsync();
                    ModelState.AddModelError(string.Empty, "Nhập bài viết mới không thành công! Vui lòng kiểm tra và thử lại!");
                    return View(model);
                }
            }
            else
