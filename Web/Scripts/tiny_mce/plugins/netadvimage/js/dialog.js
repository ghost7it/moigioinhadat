    ...
    if ($('.t-drag-status').hasClass('t-add')) { // NOTE: Asked Telerik for e.isValid
        ...
        if (dropContainer.size() > 0) {
            ...
            if (dropContainer.attr('id') === 'dir-delete') {
                tinyMCEPopup.confirm('Bạn có chắc chắn muốn xóa thư mục này và tất cả nội dung bên trong nó không?', function (s) {
                    if (s)
                        RemoveNode($(e.item));
                    else
                        e.preventDefault();
            ...
            if (dropNode.size() > 0) {
                tinyMCEPopup.confirm('Bạn có chắc chắn muốn di chuyển thư mục này không? Có thể các liên kết tới ảnh trong thư mục này sẽ bị hỏng.', function (s) {
                    if (!s)
                        e.preventDefault();
                });
            }
    ...
    if (GetNodeValue(node)) {
        ...
        $.post("_GetImages", { path: GetNodeValue(node) }, function (result) {
            ...
            if ($(result).size() > 0) {
                ...
                $('#directory img').attr('src', '/Scripts/tiny_mce/plugins/netadvimage/img/folder-horizontal.gif');
                $(node).closest('.t-item').find('img:first').attr('src', '/Scripts/tiny_mce/plugins/netadvimage/img/folder-horizontal-open.gif');
    ...
    init: function (ed) {
        ...
        var f = document.forms['mce'],
                ...
                uploader = new qq.FileUploader({
                    ...
                    action: '/Scripts/tiny_mce/plugins/netadvimage/index',
                    params: {
                        path: null
                    },
        ...
        $('#dir-delete').click(function (e) {
            if ($('.t-state-selected').size() > 0) {
                tinyMCEPopup.confirm('Bạn có chắc chắn muốn xóa thư mục và toàn bộ nội dung thư mục đã chọn không?', function (s) {
                    if (s)
                        RemoveNode($('.t-state-selected').first());
                });
            }
            else
                alert('Bạn phải chọn một thư mục để xóa.');
            e.preventDefault();
        });
        ...
        $('#dir-add').click(function (e) {
            if ($('.t-state-selected').size() > 0) {
                ...
                var cont = confirm('Chưa có thư mục nào được chọn. Bạn có muốn tạo thư mục mới vào thư mục gốc không?');
                if (!cont)
                    return false;
                else {
        ...
        $('#dir-edit').click(function (e) {
            if ($('.t-state-selected').size() > 0) {
                ...
                alert('Vui lòng chọn thư mục để đổi tên.');
            e.preventDefault();
        });
        ...
        $('#upload').live('click', function (e) {
            if ($('.t-state-selected').size() > 0) {
                ...
                alert('Bạn phải chọn một thư mục để upload ảnh lên.');
                e.preventDefault();
            }
        });
        ...
        $('#img-delete').click(function (e) {
            ...
            if ($('.t-state-selected').size() == 0) {
                alert('Bạn phải chọn một ảnh để xóa.');
                e.preventDefault();
                return;
            }
            ...
            var answer = confirm('Bạn có chắc chắn muốn xóa ảnh đã chọn không?');
            if (answer) {
